...
document.querySelector("#deleteProfile").addEventListener("click", (e) => {
  ...
  let text = "ARE YOU SURE THAT U WANT TO DELETE PROF?";
...
document.querySelector("#postForm").addEventListener("submit", (e) => {
  ...
  async function createPost() {
    ...
    <button class="comment-btn" onclick="commentPost(this)">Commments</button>
    ${delete_post_html}
</div>
</div>
...
async function getAllPosts() {
  ...
  let img = document.querySelector(".inner-container img");
    ...
    async function getPostUser() {
      ...
      if (comments.length > 0) {
        comments.forEach((comment) => {
          img.src = `https://picsum.photos/${post.user_id * 20}`;
          comments_html += `<div class="single-comment"><img class="responsive" src="https://picsum.photos/${
            post.user_id * 20
          }"><span>${user.username}</span> ${comment.content}</div>`;
        });
      }
      ...
      document.querySelector("#allPostWrapper").innerHTML =
        `<div class="single-post" data-post_id="${post.id}">
        <div class="post-actions">
        <p><img src="https://picsum.photos/${post.user_id * 20}"> ${
          user.username
        }</p>
      ...
      <button onclick="likePost(this)" class="likePostJS like-btn"><span>${
        post.likes
      }</span> Likes</button>
      <button class="comment-btn" onclick="commentPost(this)">Commments</button>
      ${delete_post_html}
...
const likePost = (btn) => {
  let main_post_el = btn.closest(".single-post");
  let post_id = btn.closest(".single-post").getAttribute("data-post_id");
  let number_of_likes = parseInt(btn.querySelector("span").innerHTML);
  btn.querySelector("span").innerHTML = number_of_likes + 1;
  btn.setAttribute("disabled", "true");
